...
description: "Patterns and architectures for autonomous Antigravity loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems."
origin: ECC
---
...
Patterns, architectures, and reference implementations for running Antigravity autonomously in loops. Covers everything from simple `claude -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
| [Continuous Antigravity PR Loop](#4-continuous-claude-pr-loop) | Medium | Multi-day iterative projects with CI gates |
...
The `claude -p` flag runs Antigravity non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
echo "Focus areas: auth module, API rate limiting" > .gemini/antigravity-context.md
claude -p "Read .gemini/antigravity-context.md for priorities. Work through them in order."
rm .gemini/antigravity-context.md
```
...
1. Loads conversation history from `~/.gemini/antigravity/claw/{session}.md`
...
### Implementation via Antigravity Commands

Create `.gemini/antigravity/commands/infinite.md`:
...
## 4. Continuous Antigravity PR Loop

**A production-grade shell script** that runs Antigravity in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
...
Antigravity reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `claude -p` invocations.

### CI Failure Recovery

When PR checks fail, Continuous Antigravity automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `claude -p` with CI fix context
3. Antigravity inspects logs via `gh run view`, fixes code, commits, pushes
4. Re-waits for checks (up to `--ci-retry-max` attempts)

### Completion Signal

Antigravity can signal "I'm done" by outputting a magic phrase:
...
└─ No → Is there a written spec/RFC?
         ...
         │        └─ No → Continuous Antigravity (iterative PR loop)
...
2. **Continuous Antigravity + De-Sloppify** — Add `--review-prompt` with a de-sloppify directive to each iteration.
...
| Continuous Antigravity | AnandChowdhary | credit: @AnandChowdhary |
