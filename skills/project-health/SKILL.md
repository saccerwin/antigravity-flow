...
| "capture learnings" / "update GEMINI.md" | Save session discoveries |
...
```
Read .gemini/antigravity/settings.local.json.
...
4. File access: check for Read/Edit/Write patterns for .gemini/antigravity/** and //tmp/**
   in project settings, and ~/Documents/**/~/.gemini/antigravity/** in global settings
...
1. Find all GEMINI.md files. For each:
...
2. Find .gemini/antigravity/rules/ topic files. Check sizes (target: 20-80 lines).
...
4. Find public markdown (README.md, LICENSE, CONTRIBUTING.md).
   Check for overlap with GEMINI.md content.

5. Check auto-memory at ~/.gemini/antigravity/projects/*/memory/MEMORY.md
...
2. Combine findings into a single report:
   ...
   **GEMINI.md quality**: [score]/100 ([grade])
...
**When**: No `.gemini/antigravity/settings.local.json` exists, or user says "setup" / "kickoff".
...
1. **Detect project type** from files present:
   ...
   | `.gemini/antigravity/agents/` or operational scripts | ops-admin | — |
   | Empty directory | Ask the user | — |

   Types stack (e.g. cloudflare-worker + javascript-typescript).

2. **Generate `.gemini/antigravity/settings.local.json`**:
   - Read [references/permission-presets.md](references/permission-presets.md)
   - Always include Universal Base (includes file access for `.gemini/antigravity/**`, `//tmp/**`)
   - Add detected language + deployment presets
   - Check if global `~/.gemini/antigravity/settings.local.json` has home-relative file access
     patterns (`~/Documents/**`, `~/.gemini/antigravity/**`). If not, suggest adding them there
...
4. **Generate `.gitignore`**:
   - Read [references/templates.md](references/templates.md)
   - Always include `.gemini/antigravity/settings.local.json`, `.gemini/antigravity/plans/`, `.jez/screenshots/`, `.jez/artifacts/`
...
2. Read existing `.gemini/antigravity/settings.local.json`
3. Merge without duplicating
4. Remind: **session restart required**

## Mode 6: Restructure Context

**When**: Root GEMINI.md over 200 lines, "restructure memory".

1. Launch Context Auditor agent first
2. Based on findings:
   - Split oversized GEMINI.md into `.gemini/antigravity/rules/<topic>.md`
   - Extract directory-specific content to sub-directory GEMINI.md
...
| File | Target | Maximum |
|------|--------|---------|
| Root GEMINI.md | 50-150 lines | 200 |
| Sub-directory GEMINI.md | 15-50 lines | 80 |
...
| Generating GEMINI.md, .gitignore | [references/templates.md](references/templates.md) |
| Scoring GEMINI.md quality | [references/quality-criteria.md](references/quality-criteria.md) |
