...
description: Especialista profundo em Antigravity - CLI da Anthropic. Maximiza produtividade com atalhos, hooks, MCPs, configuracoes avancadas, workflows, GEMINI.md, memoria, sub-agentes, permissoes e...
risk: none
source: community
...
Especialista profundo em Antigravity - CLI da Anthropic. Maximiza produtividade com atalhos, hooks, MCPs, configuracoes avancadas, workflows, GEMINI.md, memoria, sub-agentes, permissoes e integracao com ecossistemas. Ativar para: configurar Antigravity, criar hooks, otimizar GEMINI.md, usar MCPs, criar sub-agentes, resolver erros do CLI, workflows avancados, duvidas sobre qualquer feature.
...
Voce e o especialista definitivo em Antigravity. Seu objetivo e transformar
cada sessao em uma experiencia 10x mais poderosa, rapida e inteligente.

---

## 1. Fundamentos Do Antigravity

Antigravity e a CLI oficial da Anthropic para usar Antigravity como agente de codigo
diretamente no terminal. Diferente do Antigravity.ai web, o Antigravity:
- Acessa seu filesystem diretamente
- Executa comandos bash, git, npm, etc.
- Persiste contexto via GEMINI.md e memory files
...
## 2. Antigravity.Md - O Cerebro Do Projeto

O arquivo GEMINI.md na raiz do projeto e carregado automaticamente em TODA sessao.
E a forma mais poderosa de dar contexto e instrucoes persistentes ao Antigravity.

## Hierarquia De Antigravity.Md

1. ~/.gemini/antigravity/GEMINI.md          global, carregado em todo projeto
2. /projeto/GEMINI.md           nivel de projeto
3. /projeto/subpasta/GEMINI.md  nivel de subpasta, carregado ao navegar
...
## Dicas De Antigravity.Md De Elite

- Use secao Protocolo Pre-Tarefa para garantir que o Antigravity sempre use orchestrator
...
```
~/.gemini/antigravity/projects/<hash-do-path>/memory/
...
## Memoria Ativa (Em Antigravity.Md)
...
Hooks executam comandos automaticamente em eventos do Antigravity.

## Localizacao Dos Hooks

- Global: ~/.gemini/antigravity/settings.json
- Por projeto: .gemini/antigravity/settings.json (na raiz do projeto)
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo dated-action >> ~/.gemini/antigravity/action_log.txt"
          }
        ]
      }
    ]
...
```bash
cat ~/.gemini/antigravity/settings.json
python -m json.tool ~/.gemini/antigravity/settings.json   # valida o JSON
```
...
MCP (Model Context Protocol) permite adicionar ferramentas externas ao Antigravity.
Cada MCP server expoe novas ferramentas que o Antigravity pode usar nas sessoes.
...
| github | PRs, issues, commits, reviews via Antigravity |
| postgres / sqlite | Consultas SQL diretas sem sair do Antigravity |
...
O Antigravity pode criar sub-agentes via Task tool para trabalho paralelo.
...
5. Passar o GEMINI.md do projeto no contexto inicial do sub-agente

---

## Configurar Permissoes Por Projeto (.Antigravity/Settings.Json)
...
## Pipeline Ci/Cd Com Antigravity
...
| Hook nao executa | JSON invalido em settings.json | python -m json.tool ~/.gemini/antigravity/settings.json |
...
```bash
ls ~/.gemini/antigravity/projects/
ls ~/.gemini/antigravity/projects/<hash>/
cat ~/.gemini/antigravity/projects/<hash>/*.jsonl | python -m json.tool
```

---

## ~/.Antigravity/Settings.Json Completo E Recomendado
...
## Como Antigravity Se Integra Com As Skills Auri

1. GEMINI.md global lista todas as skills disponiveis e quando usar cada uma
