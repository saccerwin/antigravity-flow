...
description: >
  ...
  OAuth credentials, authentication, and installing 90+ agent skills for Antigravity.
...
Set up the `gws` CLI (@googleworkspace/cli) with OAuth credentials and 90+ agent skills for Antigravity. Produces a fully authenticated CLI with skills for Gmail, Drive, Calendar, Sheets, Docs, Chat, Tasks, and more.
...
Install the 90+ gws agent skills globally for Antigravity:
...
```bash
ls ~/.gemini/antigravity/skills/gws-* | wc -l
```
...
- [gws-shared](~/.gemini/antigravity/skills/gws-shared/SKILL.md) — Auth patterns and global flags for gws commands
