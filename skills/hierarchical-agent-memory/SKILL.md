...
description: "Scoped GEMINI.md memory system that reduces context token spend. Creates directory-level context files, tracks savings via dashboard, and routes agents to the right sub-context."
risk: safe
...
Scoped memory system that gives AI coding agents a cheat sheet for each directory instead of re-reading your entire project every prompt. Root GEMINI.md holds global context (~200 tokens), subdirectory GEMINI.md files hold scoped context (~250 tokens each), and a `.memory/` layer stores decisions, patterns, and an inbox for unconfirmed inferences.

## When to Use This Skill

- Use when you want to reduce input token costs across Antigravity sessions
- Use when your project has 3+ directories and the agent keeps re-reading the same files
- Use when you want directory-scoped context instead of one monolithic GEMINI.md
...
```
project/
├── GEMINI.md              # Root context (~200 tokens)
├── .memory/
...
└── src/
    ├── api/GEMINI.md      # Scoped context for api/
...
The root GEMINI.md includes a routing section that tells the agent exactly which sub-context to load:

```markdown
## Context Routing

→ api: src/api/GEMINI.md
→ components: src/components/GEMINI.md
→ lib: src/lib/GEMINI.md
```
...
- Context file health (missing/stale/inherited GEMINI.md coverage)
...
|---|---|
| `go ham` | Set up HAM — auto-detect platform, generate GEMINI.md files |
...
| `ham route` | Add/update Context Routing section in root GEMINI.md |
...
- Keep root GEMINI.md under 60 lines / 250 tokens
- Keep subdirectory GEMINI.md files under 75 lines each
...
- Dashboard requires Node.js 18+ and reads session data from `~/.gemini/antigravity/projects/`
- Context routing detection relies on GEMINI.md read order in session JSONL files
- Does not auto-update subdirectory GEMINI.md content — you maintain those manually or via `ham audit`
