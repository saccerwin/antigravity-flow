...
- **Prompts**: Reusable, parameterised prompt templates the client can surface (e.g. in Antigravity Desktop). Register with `registerPrompt()` or equivalent.
- **Transport**: stdio for local clients (e.g. Antigravity Desktop); Streamable HTTP is preferred for remote (Cursor, cloud). Legacy HTTP/SSE is for backward compatibility.
