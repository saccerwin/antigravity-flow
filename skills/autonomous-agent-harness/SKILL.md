...
description: Transform Antigravity into a fully autonomous agent system with persistent memory, scheduled operations, computer use, and task queuing. Replaces standalone agent frameworks (Hermes, AutoGPT) by leveraging Antigravity's native crons, dispatch, MCP tools, and memory. Use when the user wants continuous autonomous operation, scheduled tasks, or a self-directing agent loop.
origin: ECC
---

# Autonomous Agent Harness

Turn Antigravity into a persistent, self-directing agent system using only native features and MCP servers.
...
Use Antigravity's built-in memory system enhanced with MCP memory server for structured data.

**Built-in memory** (`~/.gemini/antigravity/projects/*/memory/`):
...
Write to ~/.gemini/antigravity/projects/*/memory/ for cross-session recall
...
Use Antigravity's scheduled tasks to create recurring agent operations.
...
Trigger Antigravity agents remotely for event-driven workflows.
...
# GitHub webhook → dispatch → Antigravity agent → fix → PR
...
Leverage Antigravity's computer-use MCP for physical world interaction.
...
Write task queue to ~/.gemini/antigravity/projects/*/memory/task-queue.md
...
|------------------|---------------|-----|
| Gateway/Router | Antigravity dispatch + crons | Scheduled tasks trigger agent sessions |
| Memory System | Antigravity memory + MCP memory server | Built-in persistence + knowledge graph |
...
Ensure these are in `~/.gemini/antigravity.json`:
