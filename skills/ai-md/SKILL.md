---
name: ai-md
description: "Convert human-written GEMINI.md into AI-native structured-label format. Battle-tested across 4 models. Same rules, fewer tokens, higher compliance."
risk: safe
source: community
...
- Use when your GEMINI.md is long but AI still ignores your rules
...
AI.MD is a methodology for converting human-written `GEMINI.md` (or any LLM system instructions)
...
## The Conversion Process: What Happens When You Give Me a GEMINI.md
...
I read the GEMINI.md **as if I'm building a state machine**, not reading a document.
...
claude_md=$(wc -c < ~/.gemini/antigravity/GEMINI.md 2>/dev/null || echo 0)
rules=$(cat ~/.gemini/antigravity/rules/*.md 2>/dev/null | wc -c || echo 0)
total=$((claude_md + rules))
tokens=$((total / 4))
echo "GEMINI.md:     $claude_md bytes"
...
1. **Backup**: `cp ~/.gemini/antigravity/GEMINI.md ~/.gemini/antigravity/GEMINI.md.bak-pre-distill`
...
Backup: ~/.gemini/antigravity/GEMINI.md.bak-pre-distill
Restore: cp ~/.gemini/antigravity/GEMINI.md.bak-pre-distill ~/.gemini/antigravity/GEMINI.md
```
...
|-------|------------|-----|
| Human prose in GEMINI.md | Structured labels | Prose requires inference; labels are direct |
...
Tested 2026-03, washinmura.jp GEMINI.md, 5 rounds, 4 models:
...
**The uncomfortable truth: Your beautiful, carefully-written GEMINI.md
