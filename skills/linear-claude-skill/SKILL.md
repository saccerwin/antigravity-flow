...
```bash
npx tsx ~/.gemini/antigravity/skills/linear/scripts/setup.ts
```
...
echo 'LINEAR_API_KEY=lin_api_your_key_here' >> ~/.gemini/antigravity/.env
...
```bash
npx tsx ~/.gemini/antigravity/skills/linear/scripts/query.ts "query { viewer { name } }"
```
