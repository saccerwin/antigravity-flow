...
1. **Run the update orchestrator script** without any flags (conversational mode):
   ```powershell
   pwsh -NoProfile -Command "& 'C:\Users\bobby\.gemini/antigravity\skills\speckit-updater\scripts\update-wrapper.ps1'"
   ```
...
5. **Execute approved action** by re-running with `-Proceed` flag:
   ```powershell
   pwsh -NoProfile -Command "& 'C:\Users\bobby\.gemini/antigravity\skills\speckit-updater\scripts\update-wrapper.ps1' -Proceed"
   ```
...
- Write permissions to .specify/ and .gemini/antigravity/ directories
