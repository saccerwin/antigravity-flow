...
- `.env` file with `APIFY_TOKEN` (at `~/.gemini/antigravity/.env`)
...
```bash
SKILL_PATH=~/.gemini/antigravity/skills/apify-ecommerce
```
...
```bash
node --env-file=~/.gemini/antigravity/.env $SKILL_PATH/reference/scripts/run_actor.js \
...
| Error | Solution |
|-------|----------|
| `APIFY_TOKEN not found` | Ensure `~/.gemini/antigravity/.env` contains `APIFY_TOKEN=your_token` |
