...
description: "Write-time code quality enforcement using Plankton — auto-formatting, linting, and Antigravity-powered fixes on every file edit via hooks."
origin: community
---

# Plankton Code Quality Skill

Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for Antigravity. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Antigravity subprocesses to fix violations the agent didn't catch.
...
Every time Antigravity edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
# Start Antigravity — hooks activate automatically
claude
```

No install command, no plugin config. The hooks in `.gemini/antigravity/settings.json` are picked up automatically when you run Antigravity in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.gemini/antigravity/hooks/` directory to your project
2. Copy `.gemini/antigravity/settings.json` hook configuration
...
Plankton's `.gemini/antigravity/hooks/config.json` controls all behavior:
