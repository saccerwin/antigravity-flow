---
name: configure-ecc
description: Interactive installer for Everything Antigravity — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
origin: ECC
---

# Configure Everything Antigravity (ECC)

An interactive, step-by-step installation wizard for the Everything Antigravity project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.
...
This skill must be accessible to Antigravity before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install everything-claude-code` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.gemini/antigravity/skills/configure-ecc/SKILL.md`, then activate by saying "configure ecc"
...
Options:
  - "User-level (~/.gemini/antigravity/)" — "Applies to all your Antigravity projects"
  - "Project-level (.gemini/antigravity/)" — "Applies only to the current project"
  - "Both" — "Common/shared items user-level, project-specific items project-level"
```

Store the choice as `INSTALL_LEVEL`. Set the target directory:
- User-level: `TARGET=~/.gemini/antigravity`
- Project-level: `TARGET=.gemini/antigravity` (relative to current project root)
- Both: `TARGET_USER=~/.gemini/antigravity`, `TARGET_PROJECT=.gemini/antigravity`
...
Options:
  ...
  - "Research & APIs" — "Deep research, Exa search, Antigravity API patterns"
...
| `claude-api` | Anthropic Antigravity API patterns: Messages, streaming, tool use, vision, batches, Agent SDK |
...
```bash
grep -rn "~/.gemini/antigravity/" $TARGET/skills/ $TARGET/rules/
grep -rn "../common/" $TARGET/rules/
grep -rn "skills/" $TARGET/skills/
```

**For project-level installs**, flag any references to `~/.gemini/antigravity/` paths:
- If a skill references `~/.gemini/antigravity/settings.json` — this is usually fine (settings are always user-level)
- If a skill references `~/.gemini/antigravity/skills/` or `~/.gemini/antigravity/rules/` — this may be broken if installed only at project level
...
- `continuous-learning-v2` references `~/.gemini/antigravity/homunculus/` directory
...
3. **Issue**: What's wrong (e.g., "references ~/.gemini/antigravity/skills/python-patterns but python-patterns was not installed")
4. **Suggested fix**: What to do (e.g., "install python-patterns skill" or "update path to .gemini/antigravity/skills/")
...
- For user-level: check `~/.gemini/antigravity/skills/<skill-name>/SKILL.md` exists
- For project-level: check `.gemini/antigravity/skills/<skill-name>/SKILL.md` exists
...
- Restart Antigravity after installing rules

### "Path reference errors after project-level install"
- Some skills assume `~/.gemini/antigravity/` paths. Run Step 4 verification to find and fix these.
- For `continuous-learning-v2`, the `~/.gemini/antigravity/homunculus/` directory is always user-level — this is expected and not an error.
