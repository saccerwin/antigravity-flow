---
name: ck
description: Persistent per-project memory for Antigravity. Auto-loads project context on session start, tracks sessions with git activity, and writes to native memory. Commands run deterministic Node.js scripts — behavior is consistent across model versions.
origin: community
version: 2.0.0
author: sreedhargs89
...
Scripts live at: `~/.gemini/antigravity/skills/ck/commands/` (expand `~` with `$HOME`).
...
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/init.mjs"
```
...
```bash
echo '<confirmed-json>' | node "$HOME/.gemini/antigravity/skills/ck/commands/save.mjs" --init
```
...
```bash
echo '<json>' | node "$HOME/.gemini/antigravity/skills/ck/commands/save.mjs"
```
...
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/resume.mjs" [arg]
```
...
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/info.mjs" [arg]
```
...
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/list.mjs"
```
...
If yes:
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/forget.mjs" [name]
```
...
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/migrate.mjs"
```
For a dry run first:
```bash
node "$HOME/.gemini/antigravity/skills/ck/commands/migrate.mjs" --dry-run
```
...
The hook at `~/.gemini/antigravity/skills/ck/hooks/session-start.mjs` must be registered in
`~/.gemini/antigravity/settings.json` to auto-load project context on session start:

```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"~/.gemini/antigravity/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
